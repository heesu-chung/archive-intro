...
Archive 페이지 내 영상은 용량을 극단적으로 줄이기 위해 영상툴을 이용한 재렌더링이 진행되었고, '모바일 서비스 디자인 프로세스' 파트의 이미지들은 모두 resize 이후에도 로딩이 매끄럽지 못하지만, 해당 이미지 집합이 프로세스 과정을 더 잘보여준다 판단했기에 삭제하지 않았습니다. 때문에 해당 컴포넌트가 있는 위치를 스크롤이 지날 때 이미지 로딩 과정에서 30여개의 이미지 로드에 약간의 버벅임이 발생합니다.
